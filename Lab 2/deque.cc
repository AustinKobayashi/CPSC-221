...
void DLL_Deque::removeDuplicates() {
  Node *current = left;
  while (current) {
      //while head is not null, create a repeat pointer
      //while repeat next date = current data
      //while loop moves repeat all the way up
      //Maybe should delete in this while statement
    Node* repeat = current;
