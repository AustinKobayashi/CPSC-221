...
void remove(int* heap, int key, int & size) {
    ...
    //loop through heap
    //runs ? times:
    //if all the elements are the same the while
    //loop will run a lot but this loop will only
    //run once
    //heapify + 1 + (sigma i=0 to n) of (3(n-i)+6)
    for(int i = 0; i < size; i++){
        
        //loop while the value equals key and size != 0
        //3(n-i) + 4
        while (heap[i] == key && size != 0){
            
            //if the array element isnt the last one
            //1
            if(i + 1 < size){
                
                //move every element in the heap up one
                // 3(size-i) + 1
                //each loop:
                //  - n < size
                //  - heap[n] = heap[n+1];
                ...
                //reduce size if array element is last one
                //1
                size --;
        }
    }
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
    ...
    //1
    int* mergedHeap = new int[newSize];
    
    //add all the elements of heap1
    //runs size1 times
    //3 * size1 + 1
    // i = 0;
    //each loop:
    ...
    //add all the elements of heap2
    //runs size2 times
    //3 * size2 + 1
    // i = 0;
    //each loop:
    //  - i < size2
    //  - mergedHeap[i + size1] = heap2[i];
    //  - i++
    for(int i = 0; i < size2; i++)
        mergedHeap[i + size1] = heap2[i];
    
    //reoragnize
    heapify(mergedHeap, newSize);
    
    //1
    return mergedHeap;
    
    //runtime = 3(size1 + size2) + heapify + 5
}
