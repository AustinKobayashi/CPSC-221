...
bool delete_node(Node*& root, int key) {
    ...
    // case 3: target has two children
        ...
        //PREDECESSOR starts at target left
        Node * pred = target->left;
        
        //move pred all the way right
        ...
        //parent is null (target is root)
        } else {
            
            //pred isnt immediately on the left
            if(pred != target->left){
                
                //node before pred
                Node* beforePred = target->left;
                while(beforePred->right != pred)
                    beforePred = beforePred->right;
                
                beforePred->right = NULL;
                
                //pred left = target left if pred has no left
                if(pred->left == NULL)
                    pred->left = target->left;
                
                //otherwise we need to set the children of the pred
                //need this for tree2
                //      5
                //  1       9
                //    4
                //   2
                else if(pred->left->key > target->left->key){
                    
                    //predLeft = leftmost child of pred
                    //need to slide this all the way since pred
                    //could have multiple children. Eg,
                    //      5
                    //  1       9
                    //    4
                    //   3
                    //  2
                    Node* predLeft = pred->left;
                    while(predLeft->left != NULL)
                        predLeft = predLeft->left;
                    
                    predLeft->left = target->left;
...
int depth( Node* root, Node* x ) {
    ...
    for(Node* n = root; n != x; n = ( x->key < n->key ? n->left : n->right))
        val ++;
